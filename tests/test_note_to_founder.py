...
from crm.note_to_founder import brand_from, draft, first_name, founder_of  # noqa: E402

PROPOSAL = "# Acme -- Prequalification Proposal\nPrepared by Strategic Marketing Insights\n\n## Engagement summary\n"
...
def test_founder_is_the_requester():
    ...
    assert brand_from(PROPOSAL) == "Strategic Marketing Insights"
...
def test_no_gap_line_when_something_was_found():
    text = draft(manifest(requester={"name": "Ada"}), "Acme", False, [], PROPOSAL)
    assert "could not find" not in text


def test_pricing_ask_gets_a_scoping_line_and_money_is_refused():
    text = draft(manifest("What is your hourly rate?", requester={"name": "Ada"}), "Acme", False, [], PROPOSAL)
    assert "we scope before we quote" in text
    with pytest.raises(ValueError):
        draft(manifest(requester={"name": "Ada"}), "Acme", False, ["We charge $5,000"], PROPOSAL)


def test_no_name_and_no_request():
    text = draft(manifest(text=""), "Acme", True, [], PROPOSAL)
    assert text.startswith("Hello —\n") and "Following our LinkedIn contact" in text


def test_word_cap():
    with pytest.raises(ValueError):
        draft(manifest(requester={"name": "Ada"}), "Acme", True, ["word " * 200], PROPOSAL)
